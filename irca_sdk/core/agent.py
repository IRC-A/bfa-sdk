...
from irca_sdk.core.paseto import verify_paseto_v4_public

class IRCAAgentExecutor(AgentExecutor):
    """
    Internal executor that maps standard A2A execution to IRCAAgent's run method.
    """
...
BFAAgentExecutor = IRCAAgentExecutor
...
class IRCADETValidationMiddleware(BaseHTTPMiddleware):
    """
...
class IRCAAgent(abc.ABC):
    """
    Abstract Base Class for IRC-A Agents.
    Inheriting from this class automatically configures an A2A server with zero-trust DET validation,
    cryptographic challenge-response registration, and prompt reduction helpers.
    ...
    def __init__(
        ...
        if not self.gateway_public_key and self.gateway_url:
            try:
                ...
                print(f"IRCAAgent Warning: Could not download gateway public key: {e}")
        ...
        self.executor = IRCAAgentExecutor(self)
        ...
        self.app.add_middleware(IRCADETValidationMiddleware, agent_instance=self)
    ...
    def reduce_prompt(
        ...
        """
        Reduces and compresses prompt/context payloads to optimize token consumption and latency
        before sending requests to LLMs or delegating across A2A nodes.

        Supports both structured chat message lists (e.g. `[{"role": "user", "content": "..."}]`)
        and unstructured text strings.

        Strategies:
        - 'sliding_window': Preserves system prompt(s) and keeps the latest N conversation messages.
        - 'truncate_text': Trims plain text prompts to approximately max_tokens (using character heuristics).
        """
        if not content:
            return content

        # 1. Process structured chat history lists
        if isinstance(content, list):
            if len(content) <= max_messages and not max_tokens:
                return content
            ...
            trimmed_conv = conversation_msgs[-max_messages:] if max_messages > 0 else conversation_msgs

            if max_tokens:
                char_limit = max_tokens * 4
                accumulated = []
                current_chars = sum(len(str(m.get("content", ""))) if isinstance(m, dict) else len(str(m)) for m in system_msgs)

                for m in reversed(trimmed_conv):
                    m_len = len(str(m.get("content", ""))) if isinstance(m, dict) else len(str(m))
                    if current_chars + m_len <= char_limit or not accumulated:
                        accumulated.append(m)
                        current_chars += m_len
                    else:
                        break
                trimmed_conv = list(reversed(accumulated))

            return system_msgs + trimmed_conv

        # 2. Process plain text prompt strings
